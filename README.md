...
- Spring Boot 백엔드의 인증·사용자·식단·운동·체중·알림 API 구현
- FastAPI AI 서버와 초기 플랜·식단 피드백·기록 재분석·AI 코치 흐름 연동
- Expo 모바일 클라이언트와 연결되는 JWT 인증 및 데이터 응답 구조 설계
- MySQL 데이터 모델링, Swagger 기반 API 검증, 통합 흐름 테스트

- 회원가입, 로그인, JWT 인증 및 토큰 갱신
- 건강 정보 온보딩과 목표 관리
- 식단, 운동, 체중 기록 및 홈 대시보드
- AI 초기 플랜 생성과 최근 플랜 조회
- 식단 AI 조언, 건강 기록 재분석, AI PT 채팅
- 다이어트 그룹 커뮤니티와 실시간 채팅
- 알림 설정 및 푸시 알림 연동 구조
